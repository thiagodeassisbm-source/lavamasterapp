...
export default function OrcamentoForm({ onClose, onSave, initialData }: OrcamentoFormProps) {
    ...
    // 1. MONTAGEM E CARREGAMENTO DE DADOS
    useEffect(() => {
        setIsMounted(true);
    ...
    // 2. FORÇAR PREENCHIMENTO DOS CAMPOS NA EDIÇÃO (RESOLVE O PROBLEMA DE CAMPOS VAZIOS)
    useEffect(() => {
        if (isMounted && initialData) {
            // Mapeia os campos para garantir que o formulário entenda
            reset({
                clienteId: initialData.clienteId || '0',
                clienteNome: initialData.clienteNome || '',
                veiculo: initialData.veiculo || '',
                dataValidade: initialData.dataValidade || new Date().toISOString().split('T')[0],
            ...
            if (initialData.clienteId === '0' || !initialData.clienteId) {
    ...
    // EVITA O ERRO REACT #425 (Hydration)
                ...
                <div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 border-b border-white/10 p-6 flex-shrink-0 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            {isEdit && <p className="text-[10px] text-slate-500 font-mono">#{initialData.id}</p>}
                        </div>
                    </div>
                        ...
                        <div className="space-y-6">
                            <div className="bg-white/5 rounded-2xl p-6 border border-white/10 space-y-4">
                                ...
                                <div>
                                    ...
                                    <select {...register('status')} className="w-full p-3 bg-slate-900 border border-white/10 rounded-xl text-white outline-none">
                            ...
                            <div className="bg-gradient-to-br from-slate-900 to-black rounded-3xl p-6 border border-white/10 shadow-2xl space-y-4">
                                ...
                                <button type="submit" disabled={isSubmitting} className="w-full mt-4 py-4 bg-gradient-to-r from-orange-500 to-red-600 text-white font-black rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50">
