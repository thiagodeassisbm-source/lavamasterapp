...
// Esquema mais tolerante para evitar erros bobos de validação
...
export default function OrcamentoForm({ onClose, onSave, initialData }: OrcamentoFormProps) {
    ...
    useEffect(() => {
        if (isMounted && dataLoaded && initialData) {
            ...
            reset({
                ...
                clienteNome: initialData.clienteNome || initialData.cliente?.nome || '',
                veiculo: veiculoMemo,
                dataValidade: initialData.validade?.split('T')[0] || initialData.dataValidade || new Date().toISOString().split('T')[0],
        ...
        } else if (isMounted && !isEdit && !watch('dataOrcamento' as any)) {
            // Define data padrão apenas no cliente para evitar hydration error
            setValue('dataValidade', new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]);
        }
    }, [isMounted, dataLoaded, initialData, clientes, reset, isEdit, setValue, watch]);
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
            <div className="glass-effect rounded-3xl w-full max-w-7xl max-h-[90vh] overflow-hidden border border-white/20 flex flex-col shadow-2xl bg-slate-900/95">
                <div className="bg-gradient-to-r from-orange-500/20 to-red-500/20 border-b border-white/10 p-6 flex justify-between items-center">
                    <div className="flex items-center gap-3">
                        ...
                        <div><h2 className="text-xl font-bold text-white uppercase">{isEdit ? 'Editar Orçamento' : 'Novo Orçamento'}</h2></div>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-xl transition-all"><X className="w-6 h-6 text-slate-400" /></button>
                </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-white/5 rounded-2xl p-6 border border-white/10 space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    {!isEdit && (
                                        <button type="button" onClick={() => { setIsClienteCadastrado(!isClienteCadastrado); setValue('clienteId', isClienteCadastrado ? '0' : ''); }} className="text-xs text-orange-400 underline underline-offset-4">
                                            {isClienteCadastrado ? 'Não cadastrado?' : 'Cadastrados'}
                                        </button>
                                    )}
                                </div>
                                {isClienteCadastrado ? (
                                    <select {...register('clienteId')} onChange={handleClienteChange} className="w-full p-3 bg-slate-900 border border-white/10 rounded-xl text-white outline-none focus:ring-2 focus:ring-orange-500">
                                ...
                                <div className="relative"><Car className="absolute left-3 top-3.5 w-4 h-4 text-slate-500" /><input {...register('veiculo')} className="w-full p-3 pl-10 bg-slate-900 border border-white/10 rounded-xl text-white outline-none focus:ring-2 focus:ring-orange-500" placeholder="Veículo (Modelo - Placa)" /></div>
                            </div>
                            ...
                            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-4">
                                    {servicosDisponiveis.map(s => {
                                        ...
                                        return (
                                            <button key={s.id} type="button" onClick={() => handleToggleService(s)} className={`p-3 rounded-xl border text-left flex justify-between items-center transition-all ${isSelected ? 'border-orange-500 bg-orange-500/10' : 'border-white/10 hover:border-white/20'}`}>
                                    ...
                                    {fields.map((field, index) => (
                                        <div key={field.id} className="flex gap-2 items-center bg-white/5 p-3 rounded-xl border border-white/5">
                                            ...
                                            <div className="flex items-center gap-1">
                                                <input type="number" {...register(`itens.${index}.quantidade`)} className="w-14 p-1.5 bg-slate-800 rounded-lg text-center text-white text-xs" min="1" />
                                                <input type="number" {...register(`itens.${index}.precoUnitario`)} step="0.01" className="w-24 p-1.5 bg-slate-800 rounded-lg text-right text-white text-xs" />
                            ...
                            <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                                ...
                                <textarea {...register('observacoes')} rows={3} className="w-full p-4 bg-slate-900 border border-white/10 rounded-xl text-white outline-none focus:ring-2 focus:ring-orange-500 resize-none" placeholder="Observações do orçamento..." />
                            </div>
                        </div>

                        <div className="space-y-6">
                            <div className="bg-white/5 rounded-2xl p-6 border border-white/10 space-y-4">
                                <div><label className="text-xs font-bold text-slate-500 uppercase mb-1 block">Validade</label><input type="date" {...register('dataValidade')} className="w-full p-3 bg-slate-900 border border-white/10 rounded-xl text-white" /></div>
                            ...
                            <div className="bg-slate-900 rounded-3xl p-6 border border-white/10 shadow-2xl space-y-4 relative overflow-hidden">
                                <h3 className="text-white font-bold text-lg mb-2">Resumo</h3>
                                ...
                                <div className="flex justify-between items-center text-slate-400 text-sm"><span>Desconto</span><input type="number" {...register('desconto')} step="0.01" className="w-24 p-2 bg-white/5 border border-white/10 rounded-lg text-right text-white" /></div>
                                <div className="border-t border-white/10 pt-4 flex justify-between text-white font-bold text-xl"><span>Total</span><span className="text-green-500 font-mono">R$ {totalFinal.toFixed(2)}</span></div>

                                {Object.keys(errors).length > 0 && (
                                    <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-xl mt-4">
                                        <p className="text-red-400 text-[10px] font-bold uppercase mb-2">Erros de Validação:</p>
                                        <ul className="space-y-1">
                                            {Object.entries(errors).map(([key, err]: any) => (
                                                <li key={key} className="text-[10px] text-red-300">
                                                    • {key === 'itens' ? 'Adicione o serviço corretamente' : (err.message || key)}
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                ...
                                <button type="submit" disabled={isSubmitting || (!dataLoaded && isEdit)} className="w-full mt-4 py-4 bg-gradient-to-r from-orange-500 to-red-600 text-white font-bold rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 group disabled:opacity-50">
                                    {isSubmitting ? <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin" /> : <Save className="w-6 h-6 group-hover:scale-110" />}
