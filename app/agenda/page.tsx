    ...
    const fetchAgendamentos = async () => {
        try {
            ...
            if (response.ok) {
                ...
                showToast('Erro', 'Erro ao carregar agendamentos.', 'error');
            }
        } catch (error) {
            console.error(error);
    ...
    const handleSaveAgendamento = async (data: any) => {
        try {
            if (selectedAgendamento) {
                ...
                // Recarrega a lista para mostrar o novo item com ID real do banco
                fetchAgendamentos();
            }
        } catch (error) {
            console.error('Erro no onSave:', error);
            throw error; // Repassa para o form tratar/mostrar erro
        }
    };
                                        ...
                                        <div className="flex-1 space-y-3">
                                            <div className="flex items-start justify-between">
                                                ...
                                                >
                                                    ...
                                                    <div className={`absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none ${agendamento.status === 'agendado' ? 'text-blue-800' :
                                                        agendamento.status === 'confirmado' ? 'text-green-800' :
                                                            agendamento.status === 'em_andamento' ? 'text-yellow-800' :
                                                                agendamento.status === 'concluido' ? 'text-purple-800' :
                                                                    'text-red-800'}`}>   <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
