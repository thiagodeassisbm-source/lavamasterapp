...
import { Package, AlertTriangle, TrendingUp, Plus, Edit, Trash2, FileText, Search } from 'lucide-react';
...
export default function EstoquePage() {
    ...
    const totalValor = produtos.reduce((acc, p) => acc + (p.estoque * p.preco), 0);
    const produtosBaixoEstoque = produtos.filter(p => p.estoque < p.estoqueMin).length;
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex-1">
                                    <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                                        ...
                                        {produto.estoque < produto.estoqueMin && (
                                            <span className="px-2 py-0.5 rounded textxs font-medium bg-yellow-500/10 text-yellow-400 border border-yellow-500/20 flex items-center gap-1">
                                    ...
                                    <div className="flex gap-4 mt-2 text-sm">
                                        <span className="text-slate-400">
                                            Estoque: <span className={produto.estoque < produto.estoqueMin ? 'text-yellow-400 font-bold' : 'text-green-400 font-bold'}>{produto.estoque}</span>
                                        </span>
                                        <span className="text-slate-400">Mínimo: {produto.estoqueMin}</span>
                                        <span className="text-slate-400">Preço: <span className="text-white font-medium">R$ {produto.preco?.toFixed(2)}</span></span>
                                    </div>
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <button
                                        ...
                                        className="p-2 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 transition-all opacity-100" // Sempre visível mobile/hover
                                        title="Excluir"
                                    >
